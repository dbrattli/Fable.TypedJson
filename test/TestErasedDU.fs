...
[<Fact>]
let ``test erased DU match on int`` () =
    classify (JInt 42) |> equal "int:42"
...
let ``test erased DU in Map`` () =
    let m: Map<string, JsonValue> =
        Map.ofList [ "name", JString "Alice"; "age", JInt 30; "score", JFloat 9.5; "active", JBool true ]
